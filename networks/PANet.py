...
def forward(supp_imgs, fore_mask, back_mask, qry_imgs, img_fts):
    """
    Args:
    ...
    for epi in range(batch_size):
        ###### Extract prototype ######
        supp_fg_fts = [[getFeatures(supp_fts[way, shot, [epi]],
                                         fore_mask[way, shot, [epi]])
                        for shot in range(n_shots)] for way in range(n_ways)]
        supp_bg_fts = [[getFeatures(supp_fts[way, shot, [epi]],
                                         back_mask[way, shot, [epi]])
                        for shot in range(n_shots)] for way in range(n_ways)]

        ###### Obtain the prototypes######
        fg_prototypes, bg_prototype = getPrototype(supp_fg_fts, supp_bg_fts)

        ###### Compute the distance ######
        prototypes = [bg_prototype,] + fg_prototypes
        dist = [calDist(qry_fts[:, epi], prototype) for prototype in prototypes]
        pred = torch.stack(dist, dim=1)  # N x (1 + Wa) x H' x W'
        outputs.append(F.interpolate(pred, size=img_size, mode='bilinear'))

        ###### Prototype alignment loss ######
        align_loss_epi = alignLoss(qry_fts[:, epi], pred, supp_fts[:, :, epi],
...
def getPrototype(fg_fts, bg_fts):
    """
...
def alignLoss(qry_fts, pred, supp_fts, fore_mask, back_mask):
    """
    ...
    for way in range(n_ways):
        ...
        for shot in range(n_shots):
            ...
            supp_dist = [calDist(img_fts, prototype) for prototype in prototypes]
